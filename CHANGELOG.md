...
- `CommandDispatcher`
  - 命令解析器支持多值参数键。
    - 现在形如 --key v1 v2 ... 的输入将被解析为 key=[v1, v2, ...]；
    - 若仅有单值，将自动退化为 key=value 保持兼容性；
    - 引号仍可用于包含空格的值，例如 --key "hello world" foo → key=["hello world", "foo"]。
  - 支持实参与命令名冲突时的自动解析终止。
    - 若用户输入 cmd1 cmd2 "cmd3" 且 "cmd3" 恰为已注册子命令名，则识别 "cmd3" 为字符串实参，不再继续命令树匹配；
    - 默认启用，确保字符串输入优先于命令路径解析。

### Enhanced

- 命令解析逻辑更精细，支持多层嵌套与边界判断，兼容用户手动加入引号包裹命令；
- 保留引号的 tokenizer 处理策略优化，使得后续命令判断中可识别 "cmd" 或 'cmd' 为非命令标识。

> 本版本进一步增强了命令参数系统的可表达性与健壮性，为将来加入自定义 tokenizer 与完整 CLI DSL 做准备。